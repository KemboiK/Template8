                        ...
                        for month in range(1, 13):
                            ...
                            if os.path.isdir(month_path):
                                for file in os.listdir(month_path):
                                    if file.endswith('.xlsx') and not file.endswith('_deduplicated.xlsx') and not file.endswith('_filed.xlsx'):
                                        file_path = os.path.join(month_path, file)
                                        new_file_path = file_path.replace('.xlsx', '_deduplicated.xlsx')
                                        filed_file_path = file_path.replace('.xlsx', '_filed.xlsx')
                                        ...
                                        try:
                                            ...
                                            # Add the 'Filed/Not filed' column
                                            df_summary = pd.read_excel(summary_file)
                                            if set(relevant_columns).issubset(df_summary.columns):
                                                df_summary['Filed/Not filed'] = df_summary.apply(
                                                    lambda row: 'Filed' if (
                                                ...
                                                df_summary.to_excel(filed_file_path, index=False)
                                            else:
                                                print(f"Relevant columns not found in {summary_file}.")
                                            ...
                                            try:
                                                ...
                                                filed_cases = df_summary['Filed/Not filed'].str.contains('Filed').sum()
